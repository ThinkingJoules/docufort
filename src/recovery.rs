    ...
    match start.tag() {
        ...
        BlockTag::StartAEBlock => {
            ...
            if let BlockTag::EndBlock = header.tag() {
                ...
                let brs = BlockReadSummary { hash_as_read,errors_corrected, block_start,corrupted_content_blocks, block: Block::A { start, middle: content, end }};
        ...
        BlockTag::StartBBlock => {
            match read_block_middle::<_,B>(reader_writer,error_correct_header,error_correct_content){
                Ok(BlockMiddleState::BBlock { middle, end, errors_corrected:ec, hash, corrupted_content_blocks }) => {
                    errors_corrected += ec;
                    let brs = BlockReadSummary { hash_as_read:hash,errors_corrected, block_start, block: Block::B { start, middle, end }, corrupted_content_blocks };
                    Ok(BlockState::Closed(brs))
                },
